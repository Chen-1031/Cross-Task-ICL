...
import os
import openai
from utils.data import load_prompts,load_dataset, get_instruction
import argparse
...
def run_few_shot_sim(source_dataset_name, target_dataset_name, model_name, device='cuda:2', k=1,
                     method='totally-cross-sim', main_dir='results'):
    ###### Load model #####
    ...
    target_instructions = get_instruction(target_dataset_name)
    ...
    if 'cross' in method:
        ...
        demo = False
        prompts_dataset = target_dataset['all_possible_pairs'][
            'sst2']  # sst2 will not be used its a placeholder as load_prompts needs a valid target_dataset_name
        # prompts_dataset = target_dataset['all_possible_pairs'][source_dataset_name]
    source_instructions = get_instruction(source_dataset_name)
    a = Tasks()
    if 'in' in method:
    ...
    for data in tqdm(prompts_dataset['prompts'],
    ...
    result_path = os.path.join(result_dir,
                               f"source_dataset={source_dataset_name}-model={model_name}-method={method}-shots={k}.csv")
...
if __name__ == "__main__":
    ...
    run_few_shot_sim(args.source_dataset_name, args.target_dataset_name, args.model_name, args.device, args.k,
                     args.method)