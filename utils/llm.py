...
def get_similar_example(source_name, source_instruction, target_example, target_prompt_id, device):
    ...
    source_example = prompter.get_input_data_with_def(source_instruction, source_data[index], source_prompt_id) + ' ' + \
                     source_data[index]['label']
...
def get_black_box_model_output(input_prompt, black_box_model_name, bb_model=None, bb_tokenizer=None, bb_device=None):
    if black_box_model_name in ['llama-2-7b', 'llama-2-13b']:
        ...
        generate = lambda prompt, max_new_tokens: call_model(prompt, model=bb_model, tokenizer=bb_tokenizer,
                                                             device=bb_device, max_new_tokens=max_new_tokens)
        ...
        try:
            response = openai.Completion.create(engine=deployment_name, prompt=input_prompt, max_tokens=10,
